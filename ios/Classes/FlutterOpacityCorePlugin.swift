    ...
    public func handle(
        ...
        case "init":
            if let args = call.arguments as? [String: Any],
                ...
                let shouldShowErrorsInWebView = args[
                    "shouldShowErrorsInWebView"] as? Bool
            {
                guard
                    let environmentEnum = OpacitySwiftWrapper.Environment(
                        rawValue: environment)
                else {
                    result(
                        FlutterError(
                            code: "INVALID_ENVIRONMENT",
                            message: "Invalid environment value", details: nil))
